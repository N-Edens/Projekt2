...
app.get('/vej2013', (req, res) => {
  client.query('SELECT vejnavn, AVG(gennemsnit) AS gennemsnit FROM vej2013 DESC LIMIT 4', (error, results) => {
    if (error) {
      throw error;
    }
...
app.get('/vej2016', (req, res) => {
  client.query('SELECT vejnavn, AVG(gennemsnit) AS gennemsnit FROM vej2016 DESC LIMIT 4', (error, results) => {
    if (error) {
      throw error;
    }
    res.json(results.rows);
...
app.get('/vej', (req, res) => {
  client.query('SELECT vejnavn, AVG(gennemsnit) AS gennemsnit FROM vej DESC LIMIT 4', (error, results) => {
    if (error) {
      throw error;
    }
    res.json(results.rows);
